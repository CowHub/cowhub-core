...
static PyObject* compiled_func(PyObject*self, PyObject* args)
{
    ...
    PyObject *py_I, *py_iter, *py_M;
    py_I = py_iter = py_M = NULL;
    ...
    py_I = get_variable("I",raw_locals,raw_globals);
    PyArrayObject* I_array = convert_to_numpy(py_I,"I");
    conversion_numpy_check_type(I_array,PyArray_UBYTE,"I");
    #define I1(i) (*((npy_ubyte*)(I_array->data + (i)*SI[0])))
    #define I2(i,j) (*((npy_ubyte*)(I_array->data + (i)*SI[0] + (j)*SI[1])))
    #define I3(i,j,k) (*((npy_ubyte*)(I_array->data + (i)*SI[0] + (j)*SI[1] + (k)*SI[2])))
    #define I4(i,j,k,l) (*((npy_ubyte*)(I_array->data + (i)*SI[0] + (j)*SI[1] + (k)*SI[2] + (l)*SI[3])))
    npy_intp* NI = I_array->dimensions;
    npy_intp* SI = I_array->strides;
    int DI = I_array->nd;
    npy_ubyte* I = (npy_ubyte*) I_array->data;
    py_iter = get_variable("iter",raw_locals,raw_globals);
    int iter = convert_to_int(py_iter,"iter");
    py_M = get_variable("M",raw_locals,raw_globals);
    PyArrayObject* M_array = convert_to_numpy(py_M,"M");
    conversion_numpy_check_type(M_array,PyArray_UBYTE,"M");
    #define M1(i) (*((npy_ubyte*)(M_array->data + (i)*SM[0])))
    #define M2(i,j) (*((npy_ubyte*)(M_array->data + (i)*SM[0] + (j)*SM[1])))
    #define M3(i,j,k) (*((npy_ubyte*)(M_array->data + (i)*SM[0] + (j)*SM[1] + (k)*SM[2])))
    #define M4(i,j,k,l) (*((npy_ubyte*)(M_array->data + (i)*SM[0] + (j)*SM[1] + (k)*SM[2] + (l)*SM[3])))
    npy_intp* NM = M_array->dimensions;
    npy_intp* SM = M_array->strides;
    int DM = M_array->nd;
    npy_ubyte* M = (npy_ubyte*) M_array->data;
        /* inline code */                   
...
PyMODINIT_FUNC initsc_2739f1c0fafb0bb7062bb06fd6d958c40(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("sc_2739f1c0fafb0bb7062bb06fd6d958c40", compiled_methods);
}
